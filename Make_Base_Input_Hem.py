...
# ========================
# Load the Excel file
df = pd.read_excel("./LHS_Parameters_Base_2.xlsx", engine='openpyxl')
...
for i in range(len(array1)):
    ...
    filename = f"Hem_{array1[i]}_{array2[i]}.xlsx"
...
# ===========================
def write_from_base(T, S_w, Liq_inj_rate, H2_PP, L_SA, L_Pyr, L_Hem, L_Goe, Q_v, C_v, P_v, K_v, H_v, fid):
    if H_v == 0:
        H_v = 1.0e-16
...
CONSTRAINT initial
   ...
   /
   MINERALS
      Pyrite                {P_v}    {pow(float(10), L_SA)} m^2/g
      Quartz                {Q_v}    {pow(float(10), L_SA)} m^2/g 
      Calcite               {C_v}    {pow(float(10), L_SA)} m^2/g
      Kaolinite             {K_v}    {pow(float(10), L_SA)} m^2/g
      Goethite              1.0d-16  {pow(float(10), L_SA)} m^2/g
      Anhydrite             1.0d-16  {pow(float(10), L_SA)} m^2/g
      Pyrrhotite            1.0d-7   {pow(float(10), L_SA)} m^2/g
      Hematite              {H_v}    {pow(float(10), L_SA)} m^2/g
      Magnetite             1.0d-7   {pow(float(10), L_SA)} m^2/g
   /
END
...
def write_from_base_NR(T, S_w, Liq_inj_rate, H2_PP, fid):
    print(f"""
SIMULATION
  SIMULATION_TYPE SUBSURFACE
  PROCESS_MODELS
...
CONSTRAINT initial
   ...
   /
   MINERALS
      Pyrite                1.0d-16    1.0d-16 m^2/g
      Quartz                0.8        1.0d-16 m^2/g 
      Calcite               1.0d-16    1.0d-16 m^2/g
      Kaolinite             1.0d-16    1.0d-16 m^2/g
      Goethite              1.0d-16    1.0d-16 m^2/g
      Anhydrite             1.0d-16    1.0d-16 m^2/g
      Pyrrhotite            1.0d-16    1.0d-16 m^2/g
      Hematite              1.0d-16    1.0d-16 m^2/g
      Magnetite             1.0d-16    1.0d-16 m^2/g
   /
END
...
# print(Temp[i_test-1])
# print(PP1[i_test-1])
# print(H2_PartialP[i_test-1])

for i in range(n):
    dir = './Pflo_Input/Hem/run_' + str(i + 1)
    ...
    write_from_base(Temp[i], Sw[i], Liq_inj_rate[i], H2_PartialP[i], Log_SA[i], Logk_Pyr[i], Logk_Hem[i], Logk_Goe[i], Qua_vol[i],
                    Cal_vol[i], Pyr_vol[i], Kao_vol[i], Hem_vol[i], fid=fileID)

    write_from_base_NR(Temp[i], Sw[i], Liq_inj_rate[i], H2_PartialP[i], fid=fileID_NR)