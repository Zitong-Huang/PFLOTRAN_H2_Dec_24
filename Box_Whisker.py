...
def create_combined_plots(selected_times, filename_suffix):
    ...
    for molecule, cols in columns_to_plot.items():
        ...
        plt.legend(handles=legend_elements, title='Mineral and Time', loc=legend_position)

        # Adjust x-ticks to be directly below their corresponding boxes
        new_xticks = [positions[i] for i in range(len(unique_x_vals))]
        plt.xticks(ticks=new_xticks, labels=[f"{x:.3f}" for x in unique_x_vals], rotation=45)

        # Set x and y labels based on the molecule type
        plt.xlabel('Fe(II)/Fe(tot) Mole Ratio', fontsize=16)
        if molecule == 'H2':
            plt.ylabel('H$_2$(g) Fractional Loss', fontsize=16)
        else:
            plt.ylabel('H$_2$S Concentration [ppm]', fontsize=16)
